...
func (b *BuildKit) buildFromAppSourceFiles(ctx context.Context, r *pb.BuildRequest, w *build.BuildResponseOutputWriter) (*pb.TsuruConfig, error) {
	...
	tc := &pb.TsuruConfig{Procfile: appFiles.Procfile, TsuruYaml: appFiles.TsuruYaml}
...
func (b *BuildKit) buildFromContainerImage(ctx context.Context, r *pb.BuildRequest, w *build.BuildResponseOutputWriter) (*pb.TsuruConfig, error) {
	...
	tmpDir, cleanFunc, err := generateBuildLocalDir2(ctx, b.opts.TempDir, fmt.Sprintf("FROM %s", r.SourceImage), nil, nil)
	if err != nil {
		return nil, err
	}
	defer cleanFunc()
...
func generateBuildLocalDir2(ctx context.Context, baseDir, dockerfile string, appArchiveData io.Reader, envs map[string]string) (string, func(), error) {
	noopFunc := func() {}
